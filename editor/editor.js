...
document.getElementById("add-tag-btn").addEventListener("click", () =>{
  ...
  const tagIndex = tagContainer.children.length;

  const tagDiv = document.createElement("div");
    tagDiv.className = "tag-block";
    tagDiv.innerHTML = `
      <hr />
      <strong>Tag ${tagIndex + 1}</strong><br />
      Tag: <input name = "innerTag"> <br />
      <button type="button" class="remove-tag-btn">Remove Tag</button>
      `;

    tagContainer.appendChild(tagDiv);

    const removeBtn = tagDiv.querySelector(".remove-tag-btn");
    removeBtn.addEventListener("click", () =>{
      tagDiv.remove();
    });
...
document.getElementById("add-object-btn").addEventListener("click", () => {
  ...
  const tagBlocks = document.querySelectorAll("#tag-container .tag-block");
  tagBlocks.forEach(tagEl =>{
    const inputs = tagEl.querySelectorAll("input");
    const tag = {
      tag: inputs[0].value || ""
    };
    obj.tags.push(tag);
...
function loadObjectToForm(obj) {
  ...
  form.elements["infiniteMass"].checked = obj.infiniteMass || false;
  ...
  form.elements["drawImage"].value = obj.drawEntityImage || false;
  form.elements["imgPath"].value = obj.imagePath || "";
  form.elements["spritesheetPath"].value = obj.spritesheetPath || "";
...
updateObjectBtn.addEventListener("click", () => {
  ...
  obj.infiniteMass = form.elements["infiniteMass"].checked;
  ...
  obj.drawEntityImage = form.elements["drawImage"].checked;
